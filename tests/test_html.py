...
def test_kwargs():
    assert (
        div(
            id=4, checked=True, cls="mydiv", data_name="foo", onclick="alert(1);"
        ).render()
...
def test_text():
    ...
    assert div(text("<>", escape=False)).render() == """<div><></div>"""

    assert div(text("<>")).render() == """<div>&lt;&gt;</div>"""
...
def test_attribute_dashes():
    ...
    assert (
        div(data_a_b_c="foo", data_page_size="123", aria_foo="bar").render() == expected
    )
...
def test_boolean_attributes():
    assert (
        input_(type="checkbox", checked=True).render()
        == '<input checked="checked" type="checkbox">'
    )
    assert input_(type="checkbox", checked=False).render() == '<input type="checkbox">'
...
def test_pretty():
    assert (
        head(script("foo"), style("bar")).render()
        == """<head>
  <script>foo</script>
  <style>bar</style>
</head>"""
  ...
  <pre>test</pre>
    ...
    assert (
        div(pre("test")).render(indent="\t")
        == """<div>
\t<pre>test</pre>
</div>"""
    )

    assert div(pre("test")).render(pretty=False) == """<div><pre>test</pre></div>"""

    assert (
        div(span("hi"), span("there"), __pretty=False).render()
        == """<div><span>hi</span><span>there</span></div>"""
    )

    assert span("hi", br(), "there").render() == """<span>hi<br>there</span>"""

    assert (
        span("hi", br(__inline=False), "there").render()
        == """<span>hi\n  <br>there\n</span>"""
    )

    assert (
        p("goodbye ", i("cruel"), " world").render()
...
def test_xhtml():
    assert (
        head(script("foo"), style("bar")).render(xhtml=True)
        == """<head>
  <script>foo</script>
  <style>bar</style>
</head>"""
    ...
    assert (
        span("hi", br(), "there").render(xhtml=True) == """<span>hi<br />there</span>"""
    )

    assert span("hi", br(), "there").render() == """<span>hi<br>there</span>"""

    assert (
        span("hi", br(), "there").render(xhtml=False) == """<span>hi<br>there</span>"""
    )


def test_verbatim_attributes():
    assert (
        div(attr="{<div></div>}").render()
        == """<div attr="{&lt;div&gt;&lt;/div&gt;}"></div>"""
    )
    assert (
        div(attr=raw("{<div></div>}")).render()
        == """<div attr="{<div></div>}"></div>"""
    )
