...
class dom_tag:
    ...
    def get(self, tag=None, **kwargs):
        ...
        attrs = [
            (dom_tag.clean_attribute(attr), value) for attr, value in kwargs.items()
        ]
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, int):
            ...
            try:
                ...
                raise IndexError(f'Child with index "{key}" does not exist.')
        elif isinstance(key, str):
    ...
    def _render(self, sb, indent_level, indent_str, pretty, xhtml):
        ...
        for attribute, value in sorted(self.attributes.items()):
            ...
            val = (
                ...
                else util.escape(str(value), True)
            )
            if '"' in val:
                sb.append(f" {attribute}='{val}'")
            else:
                sb.append(f' {attribute}="{val}"')
    ...
    def __repr__(self):
        name = f"{self.__module__}.{type(self).__name__}"

        attributes_len = len(self.attributes)
        attributes = f"{attributes_len} attribute"
        if attributes_len != 1:
            attributes += "s"

        children_len = len(self.children)
        children = f"{children_len} child"
        if children_len != 1:
            children += "ren"

        return f"<{name} at {id(self):x}: {attributes}, {children}>"
    ...
    @classmethod
    def clean_pair(_cls, attribute, value):
        """
        ...
        """
        attribute = _cls.clean_attribute(attribute)
